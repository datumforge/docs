...
3. Simple Python load and read tests from the different servers using the libsql_client package. Note, I was not able to get websockets to work at first and spent a lot of time trying to figure out if this was an issue with my setups but it turned out to be that some of it was just bad error handling and reporting. This was actually pretty frustrating to see that even though a trace stack is returned with the errors the error handling is not very robust or detailed so it is not easy to know what went wrong. For Example if a table creation call was not idempotent I would get an error message like this: raise LibsqlError("WebSocket was closed", "HRANA_WEBSOCKET_ERROR")  or a 500 Internal error.  After I was able to resolve this and was doing some basic testing websockets still failed relatively frequently so I just stopped using them and switched to HTTPS and with less errors. There is no error handling or retries in the code used for testing.
...
9. Started testing scale and my first setup was just to do simple insert tests as per the documentation this is the main bottleneck as writes are serializable. The results are not very promising if the write volume will be high. I switched back to using libsql instead of HTTPS for the connection protocol. I was not able to address concurrency easily when running against sqlite locally so these tests were all run with just a single process.
    1. Running from local (Kansas City) to Turso DB (Chicago) using libsql driver on Turso with no replica. All tables were cleared before running these tests.
